...
import ImageUpload from './ImageUpload'; // <--- ASIGURĂ-TE CĂ AI ACEST IMPORT
...
type FormData = {
    ...
    parent_email: string;
    phone: string;
    notes: string;     // Notite suplimentare
    images: string[];  // Array de URL-uri pentru poze
};
...
export default function OrderForm() {
    ...
    const handleNext = () => {
        ...
        // Validare Pas 1.5 (Opțional, deci trecem direct)
        else if (step === 1.5) {
            setStep(2);
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const { error } = await supabase
                .from('orders')
                .insert([formData]); // Trimite inclusiv images[] și notes
    ...
    if (isSuccess) {
        return (
            <div className="w-full max-w-2xl mx-auto bg-zinc-900 border border-green-500/30 rounded-3xl p-10 shadow-2xl text-center">
                <div className="w-20 h-20 bg-green-500/20 rounded-full flex items-center justify-center mx-auto mb-6">
                    <Star className="w-10 h-10 text-green-500 fill-green-500 animate-pulse" />
                </div>
                <h2 className="text-3xl font-bold text-white mb-4">Comandă Primită!</h2>
                <p className="text-zinc-400 mb-8">
                    Dosarul pentru <strong>{formData.child_name}</strong> a fost trimis la Polul Nord.<br />
                    Pozele au ajuns în siguranță în albumul Moșului.
                </p>
                ...
                <AnimatePresence mode="wait" custom={step}>
                    ...
                    {step === 1 && (
                        ...
                        >
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    ...
                                    <textarea
                                        ...
                                        onChange={handleChange}
                                        rows={2}
                                        placeholder="ex: A învățat să citească"
                            ...
                            <div className="space-y-2">
                                <label className="text-sm text-zinc-400 ml-1">De îmbunătățit</label>
                                <div className="relative">
                                    ...
                                    <textarea
                                        ...
                                        onChange={handleChange}
                                        rows={2}
                                        placeholder="ex: Nu mănâncă legume"
                                        className="w-full bg-zinc-950 border border-zinc-800 rounded-xl py-3 pl-10 pr-4 text-white focus:outline-none focus:border-primary transition-colors resize-none"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <div className="space-y-2">
                                    <label className="text-sm text-zinc-400 ml-1">Dorința</label>
                                    <input
                                        type="text"
                                        name="wish"
                                        value={formData.wish}
                        ...
                        >
                            ...
                            <div className="space-y-2 pt-4">
                                ...
                                <textarea
                                    ...
                                    onChange={handleChange}
                                    rows={3}
                                    placeholder="ex: Să pronunțe numele 'Matei' mai rar, sau să menționeze că îl iubim mult."
